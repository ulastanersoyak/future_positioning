...
struct Xbus
{
  ...
  static constexpr double minusHalfPi
  ...
  {
    if (x <= -1.0)
      return static_cast<float> (minusHalfPi);
    if (x >= 1.0)
      return static_cast<float> (halfPi);
    return std::asin (x);
  }
  ...
  convertQuatToEuler ()
  {
    ...
    euler[0] = static_cast<float> (
        std::atan2 (2.0f * (quat[2] * quat[3] + quat[0] * quat[1]), dphi)
        * rad2deg);
    euler[1] = static_cast<float> (
        -asinClamped (2.0f * (quat[1] * quat[3] - quat[0] * quat[2]))
        * rad2deg);
    euler[2] = static_cast<float> (
        std::atan2 (2.0f * (quat[1] * quat[2] + quat[0] * quat[3]), dpsi)
        * rad2deg);
...
{
public:
  static void parseDataPacket (const std::vector<uint8_t> &packet,
                               Xbus &xbusData);
