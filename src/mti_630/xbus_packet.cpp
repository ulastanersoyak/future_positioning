...
void XbusPacket::feedByte(uint8_t byte) {
    ...
    if (m_extendedLength && m_buffer.size() == 5) {
        m_expectedLength = (static_cast<std::size_t>(m_buffer[4]) << 8) | m_buffer[5];
        m_lengthValid = true;
...
bool XbusPacket::isPacketComplete() const {
    ...
    std::size_t totalLength = m_extendedLength ? 7 + m_expectedLength : 5 + m_expectedLength;
...
uint8_t XbusPacket::computeChecksum(const std::vector<uint8_t>& packet) const {
    ...
    for (std::size_t i = 1; i < packet.size(); ++i) { // Exclude the preamble
        sum += packet[i];
    }
